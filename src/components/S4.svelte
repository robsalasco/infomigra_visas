...
<table class="table-fixed my-6 w-full">
  ...
  <tbody>
    <tr class="bg-white">
      ...
      <td class="border px-4 py-2">{isNaN(price_utm) ? '...waiting' : '$ ' + Math.round((2.5*price_utm)).toLocaleString('es-CL')}</td>
      <td class="border px-4 py-2">{isNaN(price_utm) ? '...waiting' : '$ ' + Math.round((3.8*price_utm)).toLocaleString('es-CL')}</td>
    </tr>
